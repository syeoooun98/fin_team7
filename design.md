...
|---|---|---|---|---|
| CZ | 캐주얼 라운지 존 | `zone-coral` | `#FF7A5C` | 1층 공간 배경/식별 |
| QA | 조용한 열람실 A | `zone-blue` | `#4C8DFF` | 2층 공간 배경/식별 |
| QB | 조용한 열람실 B | `zone-navy` | `#2C3E7A` | 2층 공간 배경/식별 (QA와 톤 구분 위해 채도↓명도↓) |
| LZ | 노트북 존 | `zone-yellow` | `#F5B942` | 2층 공간 배경/식별 |
| OH | 오픈 열람홀 | `zone-teal` | `#2BB3A3` | 3층 공간 배경/식별 |
| GS | 그룹 스터디룸 | `zone-green` | `#4CAF6D` | 3층 공간 배경/식별 |

> 6개 색 모두 배경색(20% opacity 워시)과 강조선(100% opacity, 구역 라벨/보더) 두 가지 톤으로 사용한다. 인접한 QA/QB, CZ와 배경톤이 혼동되지 않도록 최종 값은 실 구현 시 대비 검증(APCA 또는 WCAG contrast checker)을 거친다.
