...
- 문서 상태: Draft v1.0
- 작성일: 2026-07-17
- 기반 문서: `PRD.md` (Draft v1.1) — 기능·정보 구조의 근거는 전부 PRD를 따른다. 본 문서는 그 정보 구조를 **어떤 화면과 시각 언어로 보여줄 것인가**만 다룬다.
- 관련 문서: `DB.md` (데이터 구조 — 좌석 상태값 3종, 자리비움/신고 라이프사이클 등 본 문서가 시각화하는 상태 값의 출처)
...
PRD 6절/7절이 이미 "정숙 열람 6구역·310석", "좌석 상태 3종 + 이용중 하위 배지 2종"이라는 정보 구조를 확정해뒀다. 이 문서는 그 위에 다음 3가지만 새로 결정한다.

1. **실제 컬러 팔레트 확정** — PRD 6.2절의 "참고 색상"(코랄/블루/네이비/옐로우/틸/그린)을 실제 토큰 값으로 확정
...
| 원칙 | 설명 |
|---|---|
| 상태가 먼저 보인다 | 이 서비스의 존재 이유는 "좌석 회전율 문제 해소"(PRD 3절)다. 모든 화면에서 좌석 상태(예약가능/빈자리/이용중)는 다른 어떤 정보보다 시각적으로 먼저 읽혀야 한다. |
...
구역 색은 **3D 맵/층 뷰에서 공간을 구분하는 용도로만** 쓰고, 개별 좌석의 상태 표시에는 사용하지 않는다(2.2절 참고 — 두 색 체계가 섞이면 "이 좌석이 비었다는 건가 QA 구역이라는 건가"가 헷갈리기 때문에 의도적으로 분리).

| 구역 코드 | 구역명 | 토큰명 | HEX | 용도 |
|---|---|---|---|---|
| F2F1 | 제1자유열람실 | `zone-coral` | `#FF7A5C` | 2층 공간 배경/식별 |
| F2SQ | 메인스퀘어 | `zone-teal` | `#2BB3A3` | 2층 공간 배경/식별 |
| F2LB | 메인로비 | `zone-slate` | `#8A93A6` | 2층 공간 배경/식별 |
| F3R1 | 제1자료실 | `zone-blue` | `#4C8DFF` | 3층 공간 배경/식별 |
| F3R2 | 제2자료실 | `zone-navy` | `#2C3E7A` | 3층 공간 배경/식별 (F3R1과 톤 구분 위해 채도↓명도↓) |
| F4F2 | 제2자유열람실 | `zone-green` | `#4CAF6D` | 4~5층 공간 배경/식별 (복층 — 5층 뷰에도 같은 색/코드로 표시) |
| F4GR | 대학원 열람실 | `zone-purple` | `#8B5CF6` | 4층 공간 배경/식별 |
| F4CR | 1인 연구 캐럴 | `zone-yellow` | `#F5B942` | 4층 공간 배경/식별 |
| F4FT | 미래인재양성센터 | `zone-pink` | `#E0559B` | 4층 공간 배경/식별 |

> 9개 색 모두 배경색(20% opacity 워시)과 강조선(100% opacity, 구역 라벨/보더) 두 가지 톤으로 사용한다. 인접한 구역끼리 배경톤이 혼동되지 않도록 최종 값은 실 구현 시 대비 검증(APCA 또는 WCAG contrast checker)을 거친다.
>
> 2026-07-17: 기존 1~3층 가상 구역(CZ/QA/QB/LZ/OH/GS)을 실제 2~5층 방 이름 기준으로 전면 교체. floor2~5.png(실측 층별 안내도) 근거.

### 2.2 좌석 상태 색 (전 구역 공통, 구역 색과 독립)

| 상태 | 토큰명 | 배경 | 보더/아이콘 | 라벨(텍스트, 색만으로 구분 금지 원칙 적용) |
|---|---|---|---|---|
| 예약가능 (AVAILABLE) | `seat-available` | `#EAF7EE` (연한 민트) | `#2E9E52` | "가능" + 빈 의자 아이콘 |
| 빈자리 (EMPTY, buffer) | `seat-empty` | `#E8F1FF` (연한 블루) + 은은한 pulse 애니메이션 | `#3B7DDB` | "방금 비었어요" + 반짝임(sparkle) 아이콘 — 5분 경과 시 `seat-available`로 자동 전환 |
...
**구조**: 3개 층을 아이소메트릭 각도(약 30°)로 겹쳐 쌓은 "단면 건물" 형태. 좌측 상단에 층 선택 탭(1F/2F/3F)을 병행 배치해, 3D 맵에서 특정 층을 탭하거나 탭 UI로 바로 이동 가능하게 한다(3D 비주얼은 "공간감"을 위한 것이지 유일한 내비게이션 수단이 아니어야 함 — 접근성/모바일 대응).

**표시 정보 (건물 전체 뷰 단계에서는 좌석 단위가 아니라 구역 단위로 집계)**:
- 각 구역은 2.1절 구역 색으로 워시 처리된 3D 블록
- 블록 위에 구역명 + "예약가능 N / 전체 M" 텍스트 배지 (개별 좌석 색은 이 단계에서 표시하지 않음 — 310석을 3D 뷰 하나에 다 찍으면 가독성이 무너지므로, 집계 숫자로 대체하고 상세는 그리드 팝업(4.3)에서 확인)
...
건물 전체 뷰에서 특정 층을 탭하면, 해당 층의 구역들이 확대된 아이소메트릭 레이아웃으로 표시된다. 예: 2F 선택 시 QA/QB/LZ 세 구역이 나란히 보임. 각 구역 카드에는:
- 구역명, 성격 태그(예: "완전 정숙", "노트북 허용" — PRD 6.2 성격 컬럼)
- 콘센트 유무 필터 아이콘(F17 대응)
- 예약가능/빈자리/이용중 3색 미니 도넛 또는 스택 바 (구역 내부 비율 요약)

### 4.3 화면: 좌석 그리드 팝업 (핵심 화면)

구역을 클릭하면 모달/바텀시트로 해당 구역의 **모든 개별 좌석**을 grid로 표시한다. 이 화면이 PRD 요구사항 대부분이 실제로 충족되는 지점이다.

**레이아웃**: 좌석 ID 순서(`CZ-001` ~ `CZ-030`처럼)로 grid 배치. 그리드 셀 하나 = 좌석 하나.
...
| 빈자리(5분 이내) | `seat-empty` 배경(pulse) + 좌석 번호 + sparkle 아이콘 |
...
**GS 구역 예외**: 그리드 상단에 룸 선택 드롭다운/탭(01~10)을 추가로 두고, 하나의 룸을 선택하면 4석 미니 그리드가 표시된다(DB.md 2.3절의 `room_number`에 대응 — 조회 편의용이며 예약 단위는 아님, PRD 14.3 확정 사항).

**필터(Should, F17)**: 그리드 상단에 "콘센트만 보기" 토글, LZ 진입 시에는 전 좌석 콘센트 보유(PRD 6.3)라 이 필터를 노출하지 않는다.
...
| 좌석 상태 | 보는 사람 | 노출 액션 |
|---|---|---|
| 예약가능 / 빈자리 | 누구나(체크인 안 한 상태) | "이 자리에 QR을 스캔해 체크인하세요" 안내 (앱 내 원격 체크인 버튼은 없음 — 반드시 현장 QR, PRD 2.3) |
| 이용중 - 재실 | 본인 | 체크아웃 버튼, 자리비움 버튼(쿨다운 중이면 잔여시간과 함께 비활성화 — F8) |
| 이용중 - 외출 | 본인 | "자리 복귀"는 자리비움 카운트다운 화면(4.5)에서 처리, 여기서는 카테고리/잔여시간 표시 |
| 이용중 - 재실 | 타인 | "신고하기" 버튼만 노출(F9) |
| 이용중 - 외출 | 타인 | **액션 없음.** "외출" 배지 외 아무 정보/버튼도 없음(F7 — 신고 버튼 자체를 렌더링하지 않음) |
...
- 전체 좌석 310석 기준 예약가능/빈자리/이용중 3색 도넛 + 숫자
...
2. 좌석 그리드에서 310석 중 대형 구역(QA/QB 60석, OH 80석)의 그리드 스크롤/페이지네이션 UX — 한 화면에 다 안 들어올 가능성, 실제 화면 크기 프로토타입으로 검증 필요
3. 구역 색 6종 + 상태 색 3~4종 합성 시 실제 대비비 수치 검증(2.1/7절) — 디자인 툴에서 목업 제작 후 확인
