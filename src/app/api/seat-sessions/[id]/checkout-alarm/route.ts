...
/**
 * POST /api/seat-sessions/[id]/checkout-alarm — "체크아웃 시 알림" 신청 (F22, DB.md 2.9절)
 * 응답 형태는 { accepted, message }로 report와 동일 계약을 따른다.
 * 중복 신청은 @@unique(seat_session_id, user_id)에 기대는 대신, 존재 여부를
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.checkoutAlarm.findUnique({
    where: { seatSessionId_userId: { seatSessionId: session.id, userId } },
  });
  if (existing) {
    return NextResponse.json({ accepted: true, message: "이미 신청된 좌석입니다." });
  }

  await prisma.checkoutAlarm.create({
    data: { seatSessionId: session.id, userId },
  });
